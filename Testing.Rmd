...
# DATA STEPS
#
#github.url <- "https://raw.githubusercontent.com/dlyork/San-Diego-County/main/san_d_dorling.geojson"
#san <- geojson_read( x=github.url,  what="sp" )
...
hist( x, breaks=50, 
      ...
      main=paste0( "Histogram of variable ", toupper( input$demographics == these.variables ) ),
...
#              choiceNames=MHV_Labels,
#              choiceValues=MHV_Values,
              selected="mhv.00")
```
...
ggplot( get_data() ) +
    geom_sf( aes( fill = q ), color=NA ) +
    coord_sf( datum=NA ) +
    labs( title = paste0( "Spatial Distribution of Home Values: ", toupper(names(MHV_Values[which(input$demographics == MHV_Values)])) ),
          caption = "Source: Harmonized Census Files",
...
get_variable_x <- reactiveValues({ d[ input$home.value ] })
...
hist( x, breaks=50, 
      col="gray", border="white", yaxt="n",
      main=paste0( "Histogram of ", toupper(names(MHV_Values[which(input$demographics == MHV_Values)])) ),
      xlab="red lines represent decile cut points" )
